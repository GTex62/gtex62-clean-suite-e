...
-- Reads from core time/calendar/astro shared cache.
  ...
  for line in s:gmatch("[^\r\n]+") do
    ...
    if line ~= "" then
      ...
      if sec then
        ...
        if k and v then
          v = v:gsub('^"', ""):gsub('"$', "")
          if section then out[section][k] = v else out[k] = v end
        end
      end
    end
  end
...
local function calendar_events_json_path() return calendar_shared_dir() .. "/events.json" end
local function calendar_status_json_path() return calendar_shared_dir() .. "/status.json" end
local function astro_current_json_path()   return astro_shared_dir()    .. "/current.json" end
local function time_current_json_path()    return time_shared_dir()     .. "/current.json" end
...
local function event_cache_path()
  ...
  return (profile.events or {}).cache_file
end

local function extra_events_path()
  local profile = calendar_profile()
  return (profile.events or {}).extra_events_file or DEFAULT_EXTRA_EVENTS
end
  ...
  for line in s:gmatch("[^\r\n]+") do
    ...
    if line ~= "" then
      ...
      if date_str and name then
        store[date_str .. "|" .. name] = { date = date_str, name = normalize_spaces(name) }
      end
    end
  end
end
...
local function parse_json_events(path, store)
  ...
  local out = command_output(string.format("jq -r '.events[]? | [.date, .name] | @tsv' %q 2>/dev/null", path))
...
local function parse_calendar_status(path)
  ...
  if not out or out == "null" then return nil end
  local state, note = out:match("^([^\t]*)\t(.*)$")
  if not state then return nil end
  return { state = normalize_spaces(state), note = normalize_spaces(note) }
end
...
local function sun_event_status_line()
  ...
  for _, c in ipairs(candidates) do
    if c.ts and c.ts > now then
      local remaining = c.ts - now
      if remaining <= SUN_EVENT_LOOKAHEAD_SECONDS and (not next_event or remaining < next_event.remaining) then
        next_event = { label = c.label, remaining = remaining }
      end
    end
  end
  if not next_event then return nil end
...
local function event_status_line()
  ...
  if sun_line then return sun_line end
  ...
  for _, event in pairs(store) do
    if event.date >= today_key and (not next_event or event.date < next_event.date or
        (event.date == next_event.date and event.name < next_event.name)) then
      next_event = event
    end
  end

  if not next_event then
    return not loaded and "EVT // CAL CACHE MISSING" or "EVT // NONE"
  end
  ...
  local suffix = delta == 0 and " TODAY" or (delta and delta > 0 and string.format(" +%dD", delta) or "")
...
local function tz_date_parts(tz)
  local out = command_output(string.format("TZ=%q date +'%%Z|%%z|%%H:%%M|%%b %%d' 2>/dev/null", tz))
  if not out then return nil end
  local zone_text, raw_off, time_text, date_text = out:match("^([^|]*)|([%+%-]%d%d%d%d)|([^|]+)|(.+)$")
  if not raw_off then return nil end
  return {
...
local function parse_time_rows(path)
  ...
  for line in out:gmatch("[^\r\n]+") do
    ...
    if zone then rows[#rows + 1] = { zone = zone, off = off, time = time_text, date = date_text, name = name } end
  end
  return rows
end
...
local function parse_time_local(path)
  ...
  return { zone = zone, time = time_text, date = date_text }
end

function M.status_lines()
  local evt_line = event_status_line()
  local local_time = parse_time_local(time_current_json_path())
  local zone = (local_time and local_time.zone ~= "" and local_time.zone) or string.upper(tostring(os.date("%Z")))
  local date_str = (local_time and local_time.date ~= "" and local_time.date) or string.upper(tostring(os.date("%a, %b %d, %Y")))
  return {
...
function M.clock_box_title() return "CLOCK" end
function M.calendar_box_title() return "CALENDAR" end
...
function M.clock_rows()
  ...
  local DISPLAY_ZONES = {
    ...
    { zone = "ET",  tz = "America/New_York",       name = "EASTERN TIME"           },
    { zone = "GMT", tz = "Europe/London",          name = "GREENWICH MEAN TIME"    },
  }
  rows = {}
...
function M.local_date()
  return string.upper(tostring(os.date("%A, %B %d, %Y")))
end
...
function M.calendar_event_dates()
  local map = {}
  for _, event in ipairs(M.calendar_events()) do map[event.date] = true end
  return map
end
